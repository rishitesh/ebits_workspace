...
class EmailConfirmationHMAC:
    ...
    htop = pyotp.HOTP('base32secret3232', 5)
    ...
    @classmethod
    def from_key_otp(cls, otp, email_address):
        email = EmailAddress.objects.get(email=email_address, verified=True)
        email_address_obj = EmailConfirmationHMAC(email)
        verified = EmailConfirmationHMAC.htop.verify(otp, int(email.counter))
        if not verified:
            email_address_obj = None
